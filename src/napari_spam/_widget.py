...
def _parse_tif_paths(paths: Iterable[Path]) -> list[ParsedTif]:
    ...
    for entry in staged:
        grouped.setdefault(entry.first_part, []).append(entry)

    parsed: list[ParsedTif] = []
    for first_part, entries in grouped.items():
...
def _parse_tsv_paths(paths: Iterable[Path]) -> list[ParsedTsv]:
    ...
    for path in paths:
        ...
        if time_key is None:
            continue
        first_part = "-".join(tokens[:consumed])
        parsed.append(ParsedTsv(path=path, first_part=first_part, time_key=time_key))
